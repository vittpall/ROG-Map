...
int main(int argc, char** argv)
{
    ...
    while(n.ok())
    {
        ...
        // hold the plant until the controller is up, so the state estimator can
        // initialize on a static drone instead of one in free fall
        if (cmd_received) quadrotor.step_forward((now_time-last_time).toSec());
        last_time = now_time;
        ...
        // accelerometer specific force: f_b = R^T (a - g), g = (0,0,-9.81)
        acc = R_body2world.transpose() * (acc + Eigen::Vector3d(0,0,9.81));
